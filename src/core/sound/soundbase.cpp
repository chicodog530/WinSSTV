...
const QString playbackStateStr[soundBase::PBEND + 1] = {
    "Playback Init",        "Playback Starting",    "Playback Running",
    "Playback Calibrate 1", "Playback Calibrate 2", "Playback End"};
...
void soundBase::run() {
  ...
  while (!stopThread) {
    ...
    case PBRUNNING:
      if (play() == 0) {
        // We do NOT stop here anymore. We just wait for more data from the
        // generator. If the generator is finished, MainWindow will call
        // drainPlayback() and idleTX().
        msleep(2);
      }
      msleep(0);
      break;
...
int soundBase::capture() {
  ...
  if (soundRoutingInput == SNDINFROMFILE) {
    ...
    // Robust check: don't abort on partial read unless it's repeatedly failing
    // or significantly off. QAudioInput sometimes returns smaller chunks.
    if (count < 4) { // Absolute minimum sanity check
      addToLog(QString("capture: very small read %1").arg(count), LOGSOUND);
      switchCaptureState(CPINIT);
      return 0;
    }

    if (count != DOWNSAMPLESIZE) {
      addToLog(QString("capture: partial read %1 vs expected %2")
                   .arg(count)
                   .arg(DOWNSAMPLESIZE),
               LOGSOUND);
      // We don't switch to CPINIT here anymore, as it causes the waterfall to
      // stop. We just process what we have.
    }

    if ((storedFrames <= (ulong)recordingSize * 1048576L) &&
        (soundRoutingInput == SNDINCARDTOFILE)) {
  ...
  // Emit audio for waterfall
  QVector<double> audioData;
  audioData.reserve(RXSTRIPE);
  for (int i = 0; i < RXSTRIPE; i++) {
    audioData.append((double)downsampleFilterPtr->filteredDataPtr()[i]);
  }
  emit processAudio(audioData);
...
int soundBase::captureCalibration(bool leadIn) {
  ...
  if (leadIn) {
    ...
    ucalibrationTime = calibrationTimer.nsecsElapsed() / 1000000000.0;
    mutex.unlock();
    // logFilePtr->addToAux(QString("%1\t%2\t%3").arg(countAvailable).arg(calibrationFrames).arg(calibrationTime)
    // );
  }
  addToLog(
      QString("read report count:%1 available %2 elapsed qtime %3, time: %4")
          .arg(count)
          .arg(countAvailable)
          .arg(calibrationTime)
...
int soundBase::playbackCalibration(bool leadIn) {
  ...
  if (leadIn) {
    ...
    if (leadInCounter == CALIBRATIONLEADIN) {
      ...
      calibrationTimer.start();
      ustartcalibrationTime = 0;
      addToLog(QString("calib start time %1").arg(ustartcalibrationTime),
               LOGCALIB);
      calibrationFrames = 0;
      mutex.unlock();
    ...
    ucalibrationTime = calibrationTimer.nsecsElapsed() / 1000000000.0;
    mutex.unlock();
    //    addToLog(QString("calib time %1 frames
    //    %2").arg(ucalibrationTime).arg(calibrationFrames),LOGCALIB);
    // logFilePtr->addToAux(QString("%1\t%2\t%3").arg(countAvailable).arg(calibrationFrames).arg(calibrationTime)
    // );
  }
  return count;
...
bool soundBase::calibrationCount(unsigned int &frames, double &elapsedTime) {
  ...
  if (frames == prevFrames)
    return false;
  prevFrames = frames;
  //  addToLog(QString("calib ok time %1 frames
  //  %2").arg(elapsedTime).arg(frames),LOGCALIB);
  return true;
}
...
bool soundBase::startCapture() {
  ...
  case SNDINFROMFILE:
    if (!waveIn.openFileForRead(audioPath, false)) {
...
int soundBase::play() {
  ...
  if (soundRoutingOutput == SNDOUTTOFILE) // output the wav-file
  {
    ...
    if (storedFrames <= (ulong)recordingSize * 1048576L) {
      waveOut.write((quint16 *)txBuffer.readPointer(), numFrames,
                    true); // always stereo
      storedFrames += numFrames;
    }
  }
  if (numFrames > 0) {
    // txBuffer.get handles ring buffer wrapping correctly. copyNoCheck did not!
    txBuffer.get((SOUNDFRAME *)tempTXBuffer, numFrames);
...
bool soundBase::startPlayback() {
  ...
  addToLog(QString("start playback, txbuffercount: %1").arg(txBuffer.count()),
           LOGSOUND);
  return true;
}
...
void soundBase::switchCaptureState(ecaptureState cs) {
  if (captureState == cs)
    return;
  qDebug() << "Switching from captureState" << captureStateStr[captureState]
           << "to" << captureStateStr[cs];
  addToLog(QString("Switching from captureState %1 to %2")
               .arg(captureStateStr[captureState])
               .arg(captureStateStr[cs]),
           LOGSOUND);
  captureState = cs;
}
...
void soundBase::switchPlaybackState(eplaybackState ps) {
  addToLog(QString("Switching from playbackState %1 to %2")
               ...
               .arg(playbackStateStr[ps]),
           LOGSOUND);
  playbackState = ps;
}

void soundBase::drainPlayback() { waitPlaybackEnd(); }
