...
/**
  ...
  \return true if the file is succcesfully opened. The file is also checked if
...
bool wavIO::openFileForRead(QString fname, bool ask) {
  ...
  if ((!checkString(waveHeader.chunkID, "RIFF")) ||
    ...
    stmb =
        new displayMBoxEvent("WAVIO Error", QString("Invalid Header Format"));
    QApplication::postEvent(dispatcherPtr, stmb);
    closeFile();
    return false;
  }

  if ((waveHeader.subChunk1Size != 16) || (waveHeader.audioFormat != 1) ||
  ...
  numberOfSamples = waveHeader.subChunk2Size /
                    (2 * numberOfChannels); // number of mono or stereo samples
  if (numberOfSamples == 0) {
    addToLog("wavio read header : number of samples = 0", LOGALL);
    stmb = new displayMBoxEvent("WAVIO Error",
                                QString("Number of samples  is zero"));
    QApplication::postEvent(dispatcherPtr, stmb);
    closeFile();
    return false;
  }
...
/**
  ...
  \return returns the number of samples read. -1 is returned if there is an
error, 0 is returned on end of the file else the number of samples read.
...
/**
  ...
  \return true if the file is succcesfully opened, and the header written, false
  otherwise?
  \sa write
*/
...
/**
  ...
  To signal the end, call this function with numSamples=0. The file will
  automatically be closed.
  \param dPtr pointer to buffer for 16 bit samples SOUNDFRAME indicates if
  samples are mono or stereo
  \param numSamples  number of samples to read
  \param isStereo false if mono,true if stereo
  \return returns true the correct number of samples are written. false
  otherwise.
*/
...
bool wavIO::write(quint16 *dPtr, uint numSamples, bool isStereo) {
  ...
  if (numSamples == 0) {
    addToLog(QString("wavio write close samples=%1").arg(numberOfSamples),
             LOGWAVIO);
    inopf.flush();
    writeHeader();
    closeFile();
  ...
  if (numberOfChannels == 1) {
    len = numSamples * sizeof(quint16);
    if (isStereo) {
      // Input is Stereo, File is Mono -> Take Left Channel
      tempBufPtr = new quint16[numSamples];
      tmpPtr = tempBufPtr;
      for (i = 0; i < numSamples; i++) {
        tempBufPtr[i] = dPtr[i * 2];
      }
    }
  } else {
    // File is Stereo
    len = numSamples * sizeof(quint16) * 2;
    if (!isStereo) {
      // Input is Mono, File is Stereo -> Expand to both channels
      tempBufPtr = new quint16[numSamples * 2];
      tmpPtr = tempBufPtr;
      for (i = 0; i < numSamples; i++) {
        tempBufPtr[i * 2] = dPtr[i];
        tempBufPtr[i * 2 + 1] = dPtr[i];
      }
    }
    // If isStereo is true, we use dPtr as is (tmpPtr already points to it)
  }
...
void wavIO::initHeader() {
  ...
  waveHeader.numChannels = (short int)numberOfChannels;
  waveHeader.sampleRate = samplingrate;
  waveHeader.byteRate = (unsigned int)numberOfChannels * 2 * samplingrate;
  waveHeader.blockAlign = (short int)numberOfChannels * 2;
  waveHeader.bitsPerSample = 16;
  waveHeader.subChunk2Size =
      numberOfSamples * sizeof(short int) * numberOfChannels;
...
bool wavIO::writeHeader() {
  int err;
  waveHeader.subChunk2Size =
      numberOfSamples * sizeof(quint16) * numberOfChannels;
  lseek(inopf.handle(), 0, SEEK_SET); // position at beginning
  if ((err = inopf.write(&waveHeader.chunkID[0], sizeof(sWave))) !=
      sizeof(sWave)) {
  ...
  addToLog(QString("wavio write header %1 %2 %3 %4")
           ...
           LOGWAVIO);
  addToLog(QString("wavio write header samples=%1").arg(numberOfSamples),
           LOGWAVIO);
  addToLog(QString("wavio write header total bytes=%1")
               .arg(numberOfSamples * 2 + sizeof(sWave)),
           LOGWAVIO);
  return true;
}
