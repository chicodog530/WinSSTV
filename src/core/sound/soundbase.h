...
#define BUFFERSIZE (64 * DOWNSAMPLESIZE)
...
public:
  ...
  void drainPlayback();
  virtual void stopImmediate() { idleTX(); }
...
protected:
  ...
  virtual int write(unsigned int numFrames) = 0;
  ...
  int sampleRate;
  qint16 tempRXBuffer[DOWNSAMPLESIZE * 2 *
                      2]; // in some cases the hardware interface is stereo (can
                          // be S16_LE or S32_LE)
