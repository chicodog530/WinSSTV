...
public:
  ...
  void sendTone(double duration, double lowerFrequency, double upperFrequency,
                bool concat);
  void sendSamples(unsigned int numSamples, double frequency);
  void sendSweep(unsigned int duration, double lowerFrequency,
                 double upperFrequency);
  void sendSilence(double duration);
  void sendSample(double freq);
  void setFilter(efilterType txFilterType);
  void writeBuffer(uint32_t *buffer, int len);
  void reset();
  void setTxClock(double clock) { txSamplingClock = clock; }
  void setWavOutput(wavIO *ptr) { wavOutput = ptr; }
