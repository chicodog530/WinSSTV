...
#define LINETOLERANCEMODEDETECT 0.05
...
sSensitivity sensitivityArray[NUMBEROFSENSITIVITIES] = {
    // minMatchedLines maxLineDistanceModeDetect maxLineDistanceInSync onRatio
    // offRatio minVolume maxTempOutOfSyncLines maxOutOfSyncLines;
    {4, 2, 7, 0.5, 0.4, 800, 10, 15},
    {4, 2, 7, 0.5, 0.4, 400, 10, 20},
    {4, 2, 7, 0.5, 0.4, 50, 15, 30},
    {4, 2, 7, 0.5, 0.3, 400, 15, 30}};

const QString syncStateStr[syncProcessor::SYNCVALID + 1] = {"SYNCOFF", "SYNCUP",
                                                            "SYNCVALID"};
 ...
 *
 * Process the received sync pulses. When a sync pulse is detected, the
 * samplecounter reference is stored in the syncArray. Concurrent with this
 * processing, the VIS decoding and FSKid decoding is done.
 *
 * \param narrow if true then this instance will use narrow settings, else wide
 * settings
 * \param parent
 */

syncProcessor::syncProcessor(bool narrow, QObject *parent)
    : QObject(parent), streamDecode(narrow) {
  detectNarrow = narrow;
  currentModePtr = NULL;
  visMode = NOTVALID;
  ...
  if (!detectNarrow) {
    bool ok1 =
        connect(streamDecode.getFskDecoderPtr(), SIGNAL(callReceived(QString)),
                SLOT(slotNewCall(QString)));
    bool ok2 = connect(streamDecode.getVisDecoderPtr(),
                       SIGNAL(visCodeWideDetected(int, uint)),
                       SLOT(slotVisCodeDetected(int, uint)));
    qInfo() << "syncProcessor Wide: connects ok1=" << ok1 << "ok2=" << ok2;
  } else {
    bool ok3 = connect(streamDecode.getVisDecoderPtr(),
                       SIGNAL(visCodeNarrowDetected(int, uint)),
                       SLOT(slotVisCodeDetected(int, uint)));
    qInfo() << "syncProcessor Narrow: connect ok3=" << ok3;
  }
}

void syncProcessor::setModeRange(esstvMode start, esstvMode end) {
  idxStart = start;
  idxEnd = end;
}
...
void syncProcessor::reset(bool preserveImage) {
  sampleCounter = 0;
  init(preserveImage);
  streamDecode.reset();
 ...
 * idxStart and idxEnd are set for narrow or wide modes in Auto mode, else
 * idxStart=idxEnd= selected mode enableSyncDetection is set to false for AVT
 * modes.
 *
 */
void syncProcessor::init(bool preserveImage) {
  ...
  preferredMode = NOTVALID;

  if (sstvModeIndexRx != NOTVALID) {
    preferredMode = sstvModeIndexRx;
    if ((preferredMode >= AVT24) && (preferredMode <= AVT94)) {
...
void syncProcessor::process() {
  ...
  if (!detectNarrow) {
    ...
    streamDecode.process(freqPtr, energy1100Ptr, energy1300Ptr,
                         sampleCounter); // only VIS and FSK if not narrow
    extractSync();
#ifdef DISABLENARROW
  }
#endif
...
#ifndef QT_NO_DEBUG
  if (detectNarrow) {
    // scopeViewerSyncNarrow->addData(// SCDATA, // syncVolumePtr,
    // sampleCounter, RXSTRIPE);
    //      // scopeViewerSyncNarrow->addData(//
    //      SCDATA,inputVolumePtr,sampleCounter,// RXSTRIPE);
    // scopeViewerSyncNarrow->addData(// SCDATA, // inputVolumePtr,
    // sampleCounter, RXSTRIPE);
    // scopeViewerSyncNarrow->addData(// SCDATA, // syncStateBuffer,
    // sampleCounter, RXSTRIPE);
    // scopeViewerSyncNarrow->addData(// SCDATA, // freqPtr, sampleCounter, //
    // RXSTRIPE);
  } else {
    // scopeViewerSyncWide->addData(// SCDATA, // syncVolumePtr, sampleCounter,
    // RXSTRIPE);
    //      // scopeViewerSyncWide->addData(//
    //      SCDATA,syncAvgPtr,sampleCounter,// RXSTRIPE);
    // scopeViewerSyncWide->addData(// SCDATA, // inputVolumePtr, sampleCounter,
    // RXSTRIPE);
    // scopeViewerSyncWide->addData(// SCDATA, // syncStateBuffer,
    // sampleCounter, RXSTRIPE);
    // scopeViewerSyncWide->addData(// SCDATA, // freqPtr, sampleCounter, //
    // RXSTRIPE);
  }
#endif
}
...
void syncProcessor::extractSync() {
  ...
  for (i = 0; i < RXSTRIPE; i++) {
    ...
    case SYNCOFF: {
      ...
      // MMSSTV-style robust sync: 1200Hz energy must exceed 1900Hz energy
      bool isSyncFreq = true;
      if (energy1200Ptr && energy1900Ptr) {
        if (energy1200Ptr[i] <= energy1900Ptr[i]) {
          isSyncFreq = false;
        }
      }
      ...
      if (isSyncFreq &&
          (syncVolumePtr[i] >
           sensitivityArray[sensitivity].onRatio * inputVolumePtr[i])) {
    ...
    case SYNCACTIVE:
      ...
      if (syncVolumePtr[i] <
          sensitivityArray[sensitivity].offRatio * inputVolumePtr[i]) {
    ...
    // Debug logging for volume levels occasionally (not every sample to avoid
    // flooding)
    if (sampleCounter % 12000 == 0 && i == 0) {
      qInfo() << "syncProcessor samples:" << sampleCounter
              << "minVol:" << sensitivityArray[sensitivity].minVolume
              << "inputVol[0]:" << inputVolumePtr[0]
              << "syncVol[0]:" << syncVolumePtr[0] << "ratio:"
              << (inputVolumePtr[0] > 0 ? syncVolumePtr[0] / inputVolumePtr[0]
                                        : 0)
  ...
  if ((syncProcesState == INSYNC) && (enableSyncDetection)) {
    if (activeChainPtr == NULL) {
      ...
      if ((sampleCounter + RXSTRIPE - RXSTRIPE / 7) >
          (lastSync + sensitivityArray[sensitivity].maxTempOutOfSyncLines *
                          samplesPerLine)) {
        if (sensitivity != (NUMBEROFSENSITIVITIES - 1))
          tempOutOfSync = true; // no temp out of sync if DX
      }
      missingLines = (uint)round(((sampleCounter + RXSTRIPE - RXSTRIPE / 7) -
                                  (lastSync + samplesPerLine)) /
                                     samplesPerLine +
                                 1);
      calcSyncQuality();
    }
 ...
 *         true if a new syncArray entry is added and further processing is
 * necessary
 */
...
bool syncProcessor::validateSync() {
  ...
  if (syncArray[syncArrayIndex].diffStartEnd() >= MINRETRACEWIDTH) {
    ...
    if ((syncArrayIndex > 2) &&
        (syncArray[syncArrayIndex].end - syncArray[syncArrayIndex - 1].start) >=
            MINRETRACEWIDTH &&
        (syncArray[syncArrayIndex].start - syncArray[syncArrayIndex - 1].end) <=
            MINRETRACEWIDTH / 5) {
...
#endif
  {
    ...
    //      addToLog(QString("Sync rejected:%1 end:%2 width
    //      %3").arg(syncArray[syncArrayIndex].start)
    //               .arg(syncArray[syncArrayIndex].end).arg(syncArray[syncArrayIndex].diffStartEnd()),LOGSYNCREJECTED);
    result = false;
  }

  if (result) {
    qInfo() << "syncProcessor Valid Sync! start:"
            << syncArray[syncArrayIndex].start
            << "width:" << syncArray[syncArrayIndex].diffStartEnd()
            << "retrace:" << syncArray[syncArrayIndex].retrace
            << "state:" << stateStr[syncProcesState];
    checkSyncArray();
    ...
    case MODEDETECT:
      if (findMatch()) {
        ...
        if (!createModeBase()) {
          ...
          // when we have S1,S2 or SDX then we have to set the syncposition at
          // the beginning of the green line
          //          syncPosition=currentModePtr->adjustSyncPosition(syncArray[0].end)-
          //          FILTERDELAYCORRECTION; // type 1 sync end
          ...
          if (syncArray[0].retrace) {
            ...
            syncCorrected =
                (syncArray[0].start + syncArray[0].end) / 2 + syncWidth / 2;
          }

          syncPosition = currentModePtr->adjustSyncPosition(
                             syncCorrected, syncArray[0].retrace) +
                         FILTERDELAYCORRECTION; // type 2 sync end
          tempOutOfSync = false;
          slantAdjustLine = 6;
          slantAdjust(true);
          switchProcessState(INSYNC);
          addToLog(QString("Mode detected: %1, sync at %2")
                       .arg(getSSTVModeNameShort(currentMode))
                       .arg(syncPosition),
                   LOGSYNCEVAL);
        }
#endif
...
void syncProcessor::trackSyncs() {
  ...
  if (currentModeMatchChanged) {
    ...
    calculateLineNumber(activeChainPtr->last()->from,
                        activeChainPtr->last()->to);
...
void syncProcessor::slotVisCodeDetected(int mode, uint visSampleCounter) {
  qInfo() << "syncProcessor: slotVisCodeDetected mode=" << mode
          << "visSampleCounter=" << visSampleCounter;

  esstvMode absStart = detectNarrow ? STARTNARROW : STARTWIDE;
  esstvMode absEnd = detectNarrow ? ENDNARROW : ENDWIDE;

  if ((mode >= absStart) && (mode <= absEnd)) {
    visMode = (esstvMode)mode;
    if ((visMode >= AVT24) && (visMode <= AVT94)) {
      enableSyncDetection = false;
    }
    qInfo() << "syncProcessor: viscode" << visMode
            << "accepted. Range:" << absStart << "-" << absEnd;
    ...
    qInfo() << "syncProcessor: viscode" << mode
            << "REJECTED. Range:" << idxStart << "-" << idxEnd;
    visMode = NOTVALID;
  }
}
...
void syncProcessor::calcSyncQuality() {
  ...
  if (activeChainPtr->count() >= 10) {
    for (k = activeChainPtr->count() - 10; k < activeChainPtr->count() - 1;
         k++) {
      tmp = activeChainPtr->at(k)->to - activeChainPtr->at(k)->from - 1;
      tmp /=
          activeChainPtr->at(k)
              ->lineSpacing; // this gives us the number of false syncs per line
      fs += tmp;
    }
  }
  if (missingLines > 5) {
  ...
  if (syncQuality < 0)
    syncQuality = 0;
  if ((syncQuality <= 0) &&
      (sensitivity != (NUMBEROFSENSITIVITIES - 1))) // i.e DX Mode
  {
...
void syncProcessor::checkSyncArray() {
  ...
  addToLog(
      QString("Found sync: start:%1 end:%2 width %3 at %4")
 ...
 * @brief syncProcessor::findMatch
 ...
 idxStart and idxEnd indicate the sstv modes to compare the sync pulse intervals
with.
  ...
  if (enableSyncDetection) {
    ...
    for (i = idxStart; i <= idxEnd; i++) {
      if (!SSTVTable[i].enabled)
        continue; // Skip disabled modes

      syncWidth = getSyncWidth((esstvMode)i, modifiedClock);
      if (addToMatch((esstvMode)i)) {
        for (j = 0; j < matchArray[i].count(); j++) {
          qInfo() << "syncProcessor: Mode" << i << "Chain" << j
                  << "count=" << matchArray[i][j]->count()
                  << "minMatched=" << minMatchedLines;
          uint requiredLines = minMatchedLines;
          if (preferredMode != NOTVALID && i == (int)preferredMode) {
            requiredLines = 1; // Eager Fallback snap
          }
          if (matchArray[i][j]->count() >= (int)requiredLines) {
            fs = 0;
            for (k = 0; k < matchArray[i][j]->count() - 1; k++) {
              fs += matchArray[i][j]->at(k)->to -
                    matchArray[i][j]->at(k)->from - 1;
            }
            qInfo() << "syncProcessor: Mode" << i << "Chain" << j
                    << "fs=" << fs;
            if (fs < 20) {
              ...
              modeList.append(i);
              totalLinesList.append(calcTotalLines(matchArray[i][j]));
              totalFractList.append(calcTotalFract(matchArray[i][j]));
            }
          }
        }
      }
    ...
    if (changeList.count() > 0)
      qInfo() << "syncProcessor: changeList size=" << changeList.count();
    for (m = 0; m < changeList.count(); m++) {
      if ((minTotLines >= totalLinesList.at(m)) &&
          (totalFractList.at(m) <= minFract)) {
        idx = m;
    ...
    if (idx >= 0) {
      qInfo() << "syncProcessor: MATCH FOUND! idx=" << idx
              << "mode=" << modeList.at(idx) << "visMode=" << visMode;
      if (visMode != NOTVALID) {
        currentMode = visMode;
      } else {
...
bool syncProcessor::addToChain(esstvMode mode, uint fromIdx) {
  ...
  if (!lineCompare(samplesPerLine, fromIdx, syncArrayIndex, lineSpacing,
                   fract)) {
    return false;
  }

  if ((syncArray[syncArrayIndex].diffStartEnd() <
       syncWidth * MINSYNCWIDTHRATIO) ||
      (syncArray[syncArrayIndex].diffStartEnd() >
       syncWidth * MAXSYNCWIDTHRATIO) ||
  ...
  if (matchArray[mode].count() == 0) // we don't have a chain yet
  {
    ...
    matchArray[mode][0]->append(
        new smatchEntry(fromIdx, syncArrayIndex, lineSpacing, fract,
    ...
    for (i = 0; i < matchArray[mode].count(); i++) {
      if (matchArray[mode][i]->last()->to == fromIdx) {
        ...
        matchArray[mode][i]->append(new smatchEntry(
            fromIdx, syncArrayIndex, lineSpacing, fract, syncArray[fromIdx].end,
    ...
    if (!found) {
      matchArray[mode].append(new modeMatchList);
      matchArray[mode].last()->append(new smatchEntry(
          fromIdx, syncArrayIndex, lineSpacing, fract, syncArray[fromIdx].end,
          syncArray[syncArrayIndex].end));
      //          addToLog(QString("Match: mode=%1,new chain=%2 syncIndex=%3
      //          end=%4").arg(getSSTVModeNameShort(mode)).arg(matchArray[mode].count()-1).arg(syncArrayIndex).arg(syncArray[syncArrayIndex].end),LOGSYNCMATCH);
 ...
 * @return true if a match for that line (i.e fractional part is smaller than
 * lineTolerance
 */

bool syncProcessor::lineCompare(DSPFLOAT samPerLine, int srcIdx, int dstIdx,
  ...
  if (visMode != NOTVALID) {
    qInfo() << "syncProcessor: lineCompare src=" << srcIdx << "dst=" << dstIdx
            << "delta=" << delta << "samPerLine=" << samPerLine
            << "lineNumber=" << lineNumber << "fraction=" << fraction
...
bool syncProcessor::createModeBase() {
  qInfo() << "syncProcessor: createModeBase mode=" << currentMode;
  bool done = false;

  // Mid-transmission retention fix
  if (currentModePtr && currentModePtr->getMode() == currentMode) {
    qInfo() << "syncProcessor: Image preserved! Mode matches" << currentMode;
    return true;
  }
  ...
  if (currentMode != NOTVALID) {
    ...
    addToLog(
        QString("create RX mode: %1").arg(getSSTVModeNameShort(currentMode)),
        LOGSYNCSTATE);
    currentModePtr->setRxTargetImage(rxTargetImage);
    currentModePtr->init(modifiedClock);
    startImageRXEvent *ce = new startImageRXEvent(
        QSize(currentModePtr->imagePixels(), currentModePtr->imageLines()));
    ce->waitFor(&done);
    QApplication::postEvent(dispatcherPtr, ce);
    while (!done) {
...
void syncProcessor::regression(DSPFLOAT &a, DSPFLOAT &b, bool initial) {
  ...
  */
  ...
  for (; j < count; j++) {
    ...
    slantXYArray[tempCount].y =
        (DSPFLOAT)(syncArray[activeChainPtr->at(j)->to].end - endZero);
    slantXYArray[tempCount].x =
        syncArray[activeChainPtr->at(j)->to].lineNumber * samplesPerLine;
    addToLog(QString("pos: %1, x=%2 y=%3 syncIndex:%4, diff %5")
             ...
             LOGSLANT);
    if ((fabs(slantXYArray[tempCount].x - slantXYArray[tempCount].y) > 150.) &&
        (!initial)) {
      falseSlantSync++;
      continue;
    }
  ...
  b = ((tempCount)*sum_xy - (sum_x * sum_y)) /
      ((tempCount)*sum_xx - (sum_x * sum_x));
...
bool syncProcessor::slantAdjust(bool initial) {
  ...
  //  addToLog(QString("regr. params line %1 a:%2
  //  b:%3").arg(slantAdjustLine).arg(a).arg(b),LOGSLANT);
  slantAdjustLine += 5;
  if (!autoSlantAdjust)
    return false;
  if (initial) {
  ...
  if (((fabs(1. - b) > 0.00001) || (fabs(a) > 1)) && autoSlantAdjust) {
    newClock = true;
    modifiedClock *= b;
    samplesPerLine = getLineLength(
        currentMode, modifiedClock); // recalculate the samples per line
    addToLog(QString("new clock accepted: %1 a=%2,b=%3")
                 .arg(modifiedClock)
                 .arg(a)
                 .arg(b),
             LOGSLANT);
    ...
    if (syncArray[0].retrace) {
      ...
      syncCorrected = (syncArray[0].start + syncArray[0].end) / 2 +
                      getSyncWidth(currentMode, modifiedClock) / 2;
    }

    syncPosition = currentModePtr->adjustSyncPosition(syncCorrected,
                                                      syncArray[0].retrace) +
                   FILTERDELAYCORRECTION; // type 2 sync end

    recalculateMatchArray();
    addToLog(QString("slantAdjust: modified  syncpos:=%1").arg(syncPosition),
             LOGSLANT);
...
void syncProcessor::switchProcessState(esyncProcessState newState) {
  addToLog(QString("syncProcessState %1 to %2")
               .arg(stateStr[syncProcesState])
               .arg(stateStr[newState]),
           LOGSYNCPROCESSSTATE);
  if ((newState == SYNCLOSTFALSESYNC) || (newState == SYNCLOSTNEWMODE) ||
      (newState == SYNCLOSTMISSINGLINES) || (newState == SYNCLOST)) {
    newState = SYNCLOST;
  }
