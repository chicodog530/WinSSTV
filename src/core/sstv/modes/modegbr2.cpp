...
modeGBR2::modeGBR2(esstvMode m, unsigned int len, bool tx, bool narrowMode)
    : modeBase(m, len, tx, narrowMode) {}

modeGBR2::~modeGBR2() {}

void modeGBR2::setupParams(double clock) {
  visibleLineLength =
      (getLineLength(mode, clock) - fp - bp - 2 * blank - syncDuration) / 3.;
}

modeBase::embState modeGBR2::rxSetupLine() {
  // the start of the Scottie mode is always at the start of the green line

  // if(subLine==0) addToLog(QString("modeGBR2: subLine %1,
  // line=%2").arg(subLine).arg(lineCounter),DBMODES);
  switch (subLine) {
  case 0:
    debugState = stG1;
...
void modeGBR2::calcPixelPositionTable(unsigned int colorLine, bool tx) {
  ...
  for (i = 0; i < activeSSTVParam->numberOfPixels; i++) {
    pixelPositionTable[i] = (unsigned int)round(
        lineStart + (((float)(i + ofx) * visibleLineLength) /
                     activeSSTVParam->numberOfPixels));
  }
}

unsigned long modeGBR2::adjustSyncPosition(unsigned long syncPos0,
                                           bool isRetrace) {
#define SCOTTYCOMPENSATION 0
  if (isRetrace)
    return syncPos0 + SCOTTYCOMPENSATION;
  if (syncPos0 < (unsigned long)(fp + 2 * visibleLineLength + 2 * blank +
                                 syncDuration / 2)) {
    return syncPos0 + (unsigned long)(bp + visibleLineLength) +
           SCOTTYCOMPENSATION;
  } else {
    return syncPos0 -
           (unsigned long)(fp + 2 * visibleLineLength + 2 * blank +
                           syncDuration) +
           SCOTTYCOMPENSATION;
  }
}
