...
const QString stateStr[modeBase::MBTXGAPROBOT + 1] = {
    "ERROR",  "SETUPLINE", "PIXELS", "SYNC",  "ENDOFLINE", "EOIMAGE",
...
/**
  ...
  This function initializes all buffers and mode parameters. The localClock will
  be set to rxClk if rxClk is not zero in receive mode, else the rxClock of the
  configuration will be used. The localClock is always equal to the txClock from
...
void modeBase::init(DSPFLOAT clk) {
  ...
  activeSSTVParam->pixelDuration =
      visibleLineLength / (double)activeSSTVParam->numberOfPixels;
  avgSample = 0;
  avgSampleCounter = 0;
  isRunning = false;
  //  if(transmit)
  ...
  //  else
  if (!transmit && rxImage) {
    if (rxImage->size() != QSize(activeSSTVParam->numberOfPixels,
                                 activeSSTVParam->numberOfDisplayLines)) {
      *rxImage =
          QImage(activeSSTVParam->numberOfPixels,
                 activeSSTVParam->numberOfDisplayLines, QImage::Format_RGB32);
    }
    rxImage->fill(QColor(128, 128, 128));
  }
}
...
modeBase::eModeBase modeBase::process(quint16 *demod, unsigned int syncPos,
                                      bool goToSync, unsigned int rxPos) {
  Q_UNUSED(rxPos);
  unsigned int i = 0;
  if (goToSync) {
    if (syncPos >= length) {
      addToLog(QString("modebase:process: syncPos: %1 > length %2")
                   .arg(syncPos)
                   .arg(length),
               LOGMODES);
      return MBENDOFIMAGE;
  ...
  for (; i < length; i++) {
      ...
      if (state == MBENDOFLINE) {
        ...
        if (!fastRedraw) {
          ...
          QApplication::postEvent(dispatcherPtr,
                                  ce); // Qt will delete it when done
        }
        lineCounter++;
    ...
    case MBSYNC:
      if (sampleCounter >= syncPosition) {
        //    addToLog(QString("modebase:mbsync
        //    =%1").arg(sampleCounter+rxSampleCounter),LOGMODES);
        switchState(MBSETUPLINE);
      }
      break;
    case MB1500: {
      ...
      if (sampleCounter >= marker) {
        ...
        if (avgFreqGapCounter >= AVGFRQOFFSET)
          avgFreqGap += demod[i];
        avgFreqGapCounter++;
        logFilePtr->addToAux(QString("m15\t%1\t%2\t%3")
                                 .arg(demod[i])
                                 .arg(avgFreqGapCounter)
                                 .arg(avgFreqGap));
      }
    } break;
    case MB2300: {
      ...
      if (sampleCounter >= marker) {
        logFilePtr->addToAux(
            QString("m23avg\t%1\t%2\t%3")
                .arg(avgFreqGap)
                .arg(avgFreqGapCounter - AVGFRQOFFSET)
                .arg(avgFreqGap / (avgFreqGapCounter - AVGFRQOFFSET)));
        avgFreqGap /= (avgFreqGapCounter - AVGFRQOFFSET);
        addToLog(
            QString("GapCounter2 %1 at %2").arg(avgFreqGap).arg(sampleCounter),
            LOGMODES);
        if (avgFreqGap < 1700) {
          addToLog(QString("Switching to 1500 GapCounter2 %1 at %2")
                       .arg(avgFreqGap)
                       .arg(i + rxPos),
                   LOGMODES);
        ...
        if (avgFreqGapCounter >= AVGFRQOFFSET)
          avgFreqGap += demod[i];
        avgFreqGapCounter++;
        logFilePtr->addToAux(QString("m23\t%1\t%2\t%3")
                                 .arg(demod[i])
                                 .arg(avgFreqGapCounter)
                                 .arg(avgFreqGap));
      }
    ...
    default:
      addToLog(QString("unknown state in modeBase: %1 receive").arg((int)state),
               LOGMODES);
      break;
    }
    sampleCounter++;
...
bool modeBase::getPixels() {
  ...
  avgSampleCounter++;
  if (sampleCounter >=
      pixelPositionTable[pixelCounter] + (activeSSTVParam->pixelDuration / 2)) {
    //      addToLog(QString("modebase:getPixels[0]
    //      =%1").arg(sampleCounter+rxSampleCounter),LOGMODES);
...
/**
  ...
  This function is the default behaviour. it calls combineColors(). This
  function must be reimplemented in the derived classes for all other colour
  modes.
*/
...
/**
  ...
  Combine  R, G and B arrays (like in Martin mode) into the rxImage and advances
  the displayCounter
*/
...
void modeBase::combineColors() {
  ...
  if (!rxImage)
    return;
  QRgb *pixelArray = (QRgb *)rxImage->scanLine(displayLineCounter);
...
/**
  ...
  Combine  Y, U  and V arrays (like in PD modes) into the rxImage and advances
  the displayCounter
*/
...
void modeBase::yuvConversion(unsigned char *array) {
  ...
  for (i = 0; i < activeSSTVParam->numberOfPixels; i++) {
    ...
    g = (100 * array[i] - 71 * redArrayPtr[i] - 33 * blueArrayPtr[i] + 13260) /
        100;
    //      r=b=g=array[i]; //test
    r = (r > 255 ? 255 : r);
    r = (r < 0 ? 0 : r);
...
modeBase::eModeBase modeBase::transmitImage(const QImage &img) {
  txImPtr = &img;
  if (img.isNull())
    return MBENDOFIMAGE;
  ...
  while (!abortRun) {
    ...
    case MBTXGAP: {
      //       addToLog(QString("MBTXGAP:
      //       samplcntr=%1").arg(sampleCounter),LOGMODES);
    ...
    case MBENDOFLINE: {
      //     addToLog(QString("MBENDOFLINE samplcntr=%1 line:
      //     %2").arg(sampleCounter).arg(lineCounter),LOGMODES);
...
/**
  ...
  This function sends one subLine of colour information contained in the
  pixelArray.
...
void modeBase::sendPixelBuffer() {
  double f;
  // addToLog (QString(" sendPixelBuffer: pixelBuffer:
  // %1").arg(QString::number((ulong)pixelArrayPtr,16)),LOGMODES);
  do {
    f = lowerFreq +
        ((double)pixelArrayPtr[pixelCounter] * (2300 - lowerFreq) / 255.);
...
/**
  ...
  Get the YUV information from the image and setup the colour arrays. The
  displayCounter is incremented by 1 or 2, depending on the mode (e.g. 1 if
  Robot24 and 2 if Robot36).
...
void modeBase::getLineY(bool evenodd) {
  ...
  if (!txImPtr)
    return;
  const QRgb *pixelArrayE = (const QRgb *)txImPtr->scanLine(displayLineCounter);
  if (evenodd) {
    const QRgb *pixelArrayO =
        (const QRgb *)txImPtr->scanLine(displayLineCounter + 1);
...
/**
  ...
  Get the luminance information from the image and setup the colour array. The
  displayCounter is incremented
...
void modeBase::getLineBW() {
  ...
  if (!txImPtr)
    return;
  const QRgb *pixelArray = (const QRgb *)txImPtr->scanLine(displayLineCounter);
  for (unsigned int i = 0; i < activeSSTVParam->numberOfPixels; i++) {
    t = pixelArray[i];
...
/**
  ...
  Get the RGB information from the image and setup the colour arrays. The
  displayCounter is incremented
...
void modeBase::switchState(embState newState) {
  //  addToLog(QString("%1 to
  //  %2").arg(stateStr[state]).arg(stateStr[newState]),LOGMODES);
  state = newState;
}
