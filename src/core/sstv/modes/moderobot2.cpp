...
modeRobot2::modeRobot2(esstvMode m, unsigned int len, bool tx, bool narrowMode)
...
void modeRobot2::setupParams(double clock) {
  // double
  // tmp=(activeSSTVParam->imageTime/(double)activeSSTVParam->numberOfDataLines)*clock;
  // visibleLineLength=(tmp-fp-bp-2*blank-syncDuration)/4;
  visibleLineLength =
      (getLineLength(mode, clock) - fp - bp - 2 * blank - syncDuration) / 4.;
}
...
modeBase::embState modeRobot2::rxSetupLine() {
  ...
  case 1:
    ...
    markerFloat += 2 * visibleLineLength; // Y is 2*C length = 92ms
    debugState = stColorLine0;
    pixelArrayPtr = yArrayPtr;
    return MBPIXELS;
  case 2:
    debugState = stG1;
    markerFloat += blank; // 4ms gap
    marker = (unsigned int)round(markerFloat);
    return MBRXWAIT;
  case 3:
    calcPixelPositionTable(REDLINE, false);
    markerFloat += visibleLineLength; // Cr is 46ms
  ...
  case 5:
    ...
    markerFloat += visibleLineLength; // Cb is 46ms
  ...
  case YLINEODD:
    for (i = 0; i < activeSSTVParam->numberOfPixels; i++) {
      pixelPositionTable[i] = (unsigned int)round(
          lineStart + (((float)(i + ofx) * 2 * visibleLineLength) /
