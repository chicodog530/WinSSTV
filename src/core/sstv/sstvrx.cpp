...
const QString stateStr[sstvRx::END + 1] = {
    "Hunting", "Processing", "WaitForSync", "SlantAdjust",
    "Restart", "Sync Lost",  "Wait",        "End"};

sstvRx::sstvRx(QObject *parent)
    : QObject(parent), syncNarrowProc(true, this), syncWideProc(false, this) {
  syncFilterPtr = NULL;
  videoFilterPtr = NULL;
  syncProcPtr = NULL;
  // #ifndef QT_NO_DEBUG
...
void sstvRx::init() {
  ...
  syncNarrowProc.setRxTargetImage(&rxImage);
  syncWideProc.setRxTargetImage(&rxImage);
  resetParams(true);
  // #ifndef QT_NO_DEBUG
  //   // scopeViewerData->setAlternativeScaleMultiplier(SUBSAMPLINGFACTOR /
  //   rxClock); // scopeViewerData->setCurveName("RX VOL", SCDATA1);
  ...
  //   // scopeViewerSyncNarrow->setAlternativeScaleMultiplier(SUBSAMPLINGFACTOR
  //   /
  //                                                        rxClock);
  //   // scopeViewerSyncWide->setAlternativeScaleMultiplier(SUBSAMPLINGFACTOR /
  //                                                      rxClock);
  // #endif
}
void sstvRx::resetParams(bool bufferReset, bool preserveImage) {
  SSTVState = HUNTING;
  bufferCounter = 0;
  if (bufferReset) {
    ...
    bufferEnergy1100.reset();
    bufferEnergy1300.reset();
    bufferInputVol.reset();
    syncNarrowProc.reset(preserveImage);
    syncWideProc.reset(preserveImage);
  } else {
    syncNarrowProc.init(preserveImage);
    syncWideProc.init(preserveImage);
  }
}
...
void sstvRx::run(DSPFLOAT *dataPtr, DSPFLOAT *volumePtr) {
  ...
  for (int i = 0; i < RXSTRIPE; i++) {
    DSPFLOAT tmp = abs(dataPtr[i]);
    if (tmp > agcVolume)
      agcVolume = tmp;
    else
      agcVolume *= DECAY;
    if (agcVolume < 1000)
      agcVolume = 1000; // maximum gain is 8 (8000/1000)
...
#endif
  bufferEnergy1100.putNoCheck(syncFilterPtr->detect1100Ptr, RXSTRIPE);
  bufferEnergy1300.putNoCheck(syncFilterPtr->detect1300Ptr, RXSTRIPE);
  ...
  while (bufferVideoDemod.count()) {
    ...
    syncWideProc.energy1100Ptr = bufferEnergy1100.readPointer();
    syncWideProc.energy1300Ptr = bufferEnergy1300.readPointer();
    syncWideProc.energy1200Ptr = bufferSync1200Vol.readPointer();
    syncWideProc.energy1900Ptr = bufferSync1900Vol.readPointer();

    syncNarrowProc.energy1100Ptr = bufferEnergy1100.readPointer();
    syncNarrowProc.energy1300Ptr = bufferEnergy1300.readPointer();
    syncNarrowProc.energy1200Ptr = bufferSync1200Vol.readPointer();
    syncNarrowProc.energy1900Ptr = bufferSync1900Vol.readPointer();
    ...
    // #ifndef QT_NO_DEBUG
    //     // scopeViewerData->addData(SCDATA1, bufferInputVol.readPointer(),
    //                              // syncWideProc.sampleCounter, RXSTRIPE);
    //     // scopeViewerData->addData(SCDATA2, dataPtr,
    //     syncWideProc.sampleCounter,
    //                              RXSTRIPE);
    //     // scopeViewerData->addData(SCDATA4, bufferVideoDemod.readPointer(),
    //                              // syncWideProc.sampleCounter, RXSTRIPE);
    // #endif
  ...
  //  sampleCounter:=%2").arg(bufferVideoDemod.getReadIndex()).arg(syncProcPtr->sampleCounter),LOGRXMAIN);
}
...
void sstvRx::advanceBuffers() {
  ...
  bufferEnergy1100.skip(RXSTRIPE);
  bufferEnergy1300.skip(RXSTRIPE);
...
void sstvRx::rewindBuffers(uint rlen) {
  ...
  bufferEnergy1100.rewind(rlen);
  bufferEnergy1300.rewind(rlen);
...
void sstvRx::process() {
  ...
  // Mode locking: If user has selected a mode, force the sync processors to
  // only use that mode.
  if (sstvModeIndexRx != NOTVALID) {
    syncWideProc.setModeRange(sstvModeIndexRx, sstvModeIndexRx);
    syncNarrowProc.setModeRange(sstvModeIndexRx, sstvModeIndexRx);
  } else {
    syncWideProc.setModeRange(M1, FAX480);
    syncNarrowProc.setModeRange(MP73N, MC180N);
  }
  ...
  if ((syncProcPtr != 0) &&
      (syncProcPtr->getSyncState(syncPosition) == syncProcessor::SYNCLOST)) {
  ...
  case HUNTING:
    if (syncProcPtr == NULL) {
      ...
      QApplication::postEvent(dispatcherPtr,
                              stce); // Qt will delete it when done
      advanceBuffers();
      break; // no sync
    }
    stce = new rxSSTVStatusEvent(QString("Receiving ") +
                                 getSSTVModeNameLong(syncProcPtr->getMode()));
  ...
  case SLANTADJUST:
    ...
    //      ri=bufferVideoDemod.getReadIndex();
    //      addToLog(QString("sc_rewind: block=%1,new readIndex=
    //      %2").arg(block).arg(ri),LOGRXFUNC);
    ...
    if (syncProcPtr->currentModePtr->process(
            bufferVideoDemod.readPointer(),
...
#ifndef QT_NO_DEBUG
    // scopeViewerData->addData(SCDATA3,
    // // syncProcPtr->currentModePtr->debugStatePtr,
    // // syncProcPtr->sampleCounter, RXSTRIPE);
#endif
    addToLog(
        QString(
            "rxFunctions: currentMode pos:=%1, syncProcPtr->sampleCounter %2")
            .arg(syncPosition - syncProcPtr->sampleCounter)
            .arg(syncProcPtr->sampleCounter),
        LOGRXFUNC);
    //      addToLog(QString("after Current mode set:
    //      %1,syncProcPtr->sampleCounter:
    ...
    while (syncProcPtr->sampleCounter < sampleCounterLatch) {
      bufferVideoDemod.skip(RXSTRIPE);
      syncProcPtr->sampleCounter += RXSTRIPE;
      //          addToLog(QString("loop readIndex:
      //          %1,syncProcPtr->sampleCounter:
      //          %2").arg(rxHoldingBuffer.getReadIndex()).arg(syncProcPtr->sampleCounter),LOGRXFUNC);
      syncProcPtr->currentModePtr->process(bufferVideoDemod.readPointer(), 0,
                                           false, syncProcPtr->sampleCounter);
      //      //
      //      scopeViewerData->addData(SCDATA2,bufferVideoDemod.readPointer(),syncProcPtr->sampleCounter,RXSTRIPE);
      // #ifndef QT_NO_DEBUG
      //       // scopeViewerData->addData(SCDATA3,
  ...
  case PROCESSING:
    if (syncState != syncProcessor::INSYNC) {
      ...
      if (syncProcPtr->currentModePtr->process(
              bufferVideoDemod.readPointer(), 0, false,
              syncProcPtr->sampleCounter) != modeBase::MBRUNNING) {
        switchState(END);
      }
    }
  ...
  case SYNCLOST:
    saveImage();
    diff = (syncProcPtr->sampleCounter - syncProcPtr->lastValidSyncCounter) /
           RXSTRIPE;
    addToLog(QString("rewind after synclost %1").arg(diff), LOGRXFUNC);
    rewindBuffers(diff * RXSTRIPE);
    syncProcPtr->resetRetraceFlag();
    resetParams(true, true); // Keep the image alive across dropout resync
    break;
  case WAIT:
    break;
  case END:
...
void sstvRx::saveImage() {
  ...
  if (syncProcPtr->currentModePtr->receivedLines() <
    ...
    endce =
        new endImageSSTVRXEvent(NOTVALID); // indicate there  is no valid image
  } else {
    //      endce = new
    //      endImageSSTVRXEvent(getSSTVModeNameShort(syncProcPtr->getMode()));
...
void sstvRx::switchState(eSSTVState newState) {
  addToLog(QString("%1 to %2").arg(stateStr[SSTVState]).arg(stateStr[newState]),
           LOGRXFUNC);
  SSTVState = newState;
}
