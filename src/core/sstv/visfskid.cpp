...
const QString fskStateStr[fskIdDecoder::GETID + 1] = {
    "FSKINIT",   "WAITS1500", "WAITE1500", "WAITS1900",
    "WAITE1900", "WAITS2100", "WAITE2100", "GETID"};

const QString visStateStr[visDecoder::GETCODE + 1] = {
    "VISINIT",   "WAITS1200", "WAITE1200", "WAITS1900", "WAITE1900",
    "WAITS2100", "WAITE2100", "WAITBIT",   "GETCODE"};
...
bool fskIdDecoder::assemble(bool reset) {
  ...
  if (endFound) {
    ...
    addToLog(
        QString("fskstr %1 %2").arg(fskIDStr).arg(QString::number(symbol, 16)),
        LOGFSKID);
...
void fskIdDecoder::extract(unsigned int syncSampleCtr, bool narrow) {
  ...
  for (i = 0; i < RXSTRIPE; i++) {
    ...
    case WAITSTART1900:
      ...
      break;
    case WAITEND1900: {
      if ((sampleCounter + i - startSampleCounter) >=
          (FSKBIT / 2 - avgCount - 56)) {
        switchState(GETID, i);
        startSampleCounter = sampleCounter + i;
        assemble(true);
        bitCounter = 0;
    ...
    case GETID:
      if ((sampleCounter + i - startSampleCounter) >= FSKBIT) {
        ...
        addToLog(QString("bit %1, %2 - %3 ")
                     .arg(bitCounter)
                     .arg(sampleCounter + i)
                     .arg(QString::number(symbol, 2)),
                 LOGFSKID);
        bitCounter++;
        if (bitCounter == 6) {
          if (assemble(false)) {
...
// c2 1200 Hz 30
visDecoder::visDecoder() : fskDecoder() {
  mode = NOTVALID;
  visState = VISINIT;
  energy1100Ptr = NULL;
  energy1300Ptr = NULL;
...
unsigned int visDecoder::getCode() { return code; }
...
void visDecoder::extract(unsigned int syncSampleCtr, bool narrow) {
  syncSampleCounter = syncSampleCtr;
  extractWide(); // Both now use energy-based if pointers set
}
...
void visDecoder::extractWide() {
  ...
  for (i = 0; i < RXSTRIPE; i++) {
    avgFreq = freqPtr[i];
    /* if (i == 0 && sampleCounter % 12000 == 0) {
      qInfo() << "visDecoder: avgFreq=" << avgFreq << "state=" << visState;
    } */
    switch (visState) {
    case VISINIT:
      sampleCounter = 0;
    ...
    case WAITSTART1900:
      if (waitStartFreq(1700, 2100)) {
        // qInfo() << "visDecoder: WAITSTART1900 matched (break/startbit)";
        switchState(WAITEND1900, i);
        startSampleCounter = sampleCounter + i;
      }
      break;
    case WAITEND1900: {
      if (waitEndFreq(1700, 2100)) {

        if (((sampleCounter + i - startSampleCounter) >= 1200) &&
            ((sampleCounter + i - startSampleCounter) < 2 * VISMIN1900)) {
          /* qInfo() << "visDecoder: WAITEND1900 Valid 1900 leader! duration:"
                  << sampleCounter + i - startSampleCounter; */
          addToLog(QString("end1900 at %1").arg(syncSampleCounter + i),
                   LOGVISCODE);
          switchState(WAITSTART1200, i);
          timeoutCounter = 0;
        } else {
          /* qInfo() << "visDecoder: WAITEND1900 Invalid 1900 leader length:"
                  << sampleCounter + i - startSampleCounter; */
          switchState(VISINIT, i);
        }
      }
    ...
    case WAITSTART1200:
      if (waitStartFreq(1000, 1400, 600, timeout)) {
        // qInfo() << "visDecoder: WAITSTART1200 matched (break/startbit)";
        addToLog(QString("start1200 at %1").arg(syncSampleCounter + i),
                 LOGVISCODE);
      ...
      break;
    case WAITEND1200: {
      if (waitEndFreq(1000, 1400, 180, timeout)) {
        switchState(VISINIT,
                    i); // shorter than 15ms is likely noise or a break pulse
      }
      if (timeout) {
        validCode = false;
        switchState(GETCODE, i);
        startSampleCounter = sampleCounter + i;
        addToLog(QString("startbit at %1").arg(syncSampleCounter + i),
                 LOGVISCODE);
        bitCounter = 0;
        bit = 1;
        symbol = 0;
    ...
    case GETCODE:
      if ((sampleCounter + i - startSampleCounter) >= VISBITWIDE) {
        /* qInfo() << "visDecoder: GETCODE bit" << bitCounter
                << "freq:" << avgFreq; */
        if (avgFreq > 1400 || (bitCounter >= 8 && bitCounter <= 11 &&
                               lookupVIS(symbol & 0xFF) != NOTVALID)) {
          // end of VIS detected or valid 8-bit code found early
          validCode = true;
          // qInfo() << "visDecoder: VIS end detected. code:" << hex << symbol;
          addToLog(QString("end bits at %1")
                       .arg(syncSampleCounter + i - VISBITWIDE / 2),
                   LOGVISCODE);
          if (bitCounter <= 11) {
            symbol &= 0xFF;
          } else {
          ...
          // MMSSTV Logic: Relative energy check is much more robust than
          // frequency estimation
          if (energy1100Ptr && energy1300Ptr) {
            if (energy1100Ptr[i] > energy1300Ptr[i]) {
              symbol |= bit; // 1 (1100Hz)
            }
          } else {
            // Fallback to frequency if pointers not set
            if (avgFreq < 1200) {
              symbol |= bit;
            }
          }

          addToLog(QString("bit %1, %2 - f:%3 e11:%4 e13:%5 s:%6")
                       .arg(bitCounter)
                       .arg(syncSampleCounter + i)
                       .arg(avgFreq)
                       .arg(energy1100Ptr ? energy1100Ptr[i] : 0)
                       .arg(energy1300Ptr ? energy1300Ptr[i] : 0)
                       .arg(QString::number(symbol, 2)),
                   LOGVISCODE);
          bitCounter++;
          bit <<= 1;
...
void streamDecoder::process(quint16 *freqPtr, FILTERPARAMTYPE *e1100,
                            FILTERPARAMTYPE *e1300,
  ...
  visCoder.setEnergyPtrs(e1100, e1300);
